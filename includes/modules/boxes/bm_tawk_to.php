...
class bm_tawk_to {
    ...
    function install()
    {
        ...
        $query = "insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Widget Script', 'MODULE_BOXES_TAWK_TO_WIDGET', '', 'The tawk.to widget script', '1', '0', 'bm_tawk_to_widget(\'\',',now())";
        tep_db_query($query);

        $query = "insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('API Key', 'MODULE_BOXES_TAWK_TO_API_KEY', '', 'Your tawk.to API Key', '6', '0', now())";
        tep_db_query($query);

        $query = "insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_BOXES_TAWK_TO_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())";
        tep_db_query($query);
    }
    ...
    function execute()
    {
        ...
        $query = 'SELECT * FROM configuration WHERE configuration_key="MODULE_BOXES_TAWK_TO_WIDGET" LIMIT 1';
